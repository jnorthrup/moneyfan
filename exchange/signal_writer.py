...
class KotlinReader:
    """
    Read commands from Kotlin via stdin
    """
    
    @staticmethod
    def read_line(timeout: float = None) -> Optional[Dict[str, Any]]:
        """
        ...
        Returns:
            Parsed JSON dict or None if timeout/no data
        """
        import select
        ...
        try:
            ...
            SignalWriter.write_error(f"Invalid JSON from Kotlin: {e}", {'line': line})
            return None
    
    @staticmethod
    def listen(callback: callable, poll_interval: float = 0.1) -> None:
        """
        Listen for commands from Kotlin and call callback
        
        Args:
            callback: Function to handle commands (must accept Dict[str, Any])
        ...
        def signal_handler(signum, frame):
            SignalWriter.write_log('INFO', 'Received shutdown signal')
            sys.exit(0)
        
        signal.signal(signal.SIGINT, signal_handler)
        signal.signal(signal.SIGTERM, signal_handler)
        
        SignalWriter.write_log('INFO', 'Starting Kotlin listener', {'poll_interval': poll_interval})
        
        while True:
            try:
                command = KotlinReader.read_line(timeout=poll_interval)
                
                if command:
                    try:
                        callback(command)
                    except Exception as e:
                        SignalWriter.write_error(f"Callback error: {e}", {'command': command})
                
                # Small delay to prevent CPU spinning
                time.sleep(0.01)
                
            except KeyboardInterrupt:
                SignalWriter.write_log('INFO', 'Keyboard interrupt received')
                break
            except Exception as e:
                SignalWriter.write_error(f"Listener error: {e}")
                time.sleep(1)  # Wait before retrying
    
    @staticmethod
    def get_stdin_mode() -> str:
        """
        Determine stdin mode (interactive vs pipe)
        
        Returns:
            'interactive', 'pipe', or 'closed'
        """
        import select
    ...
    def read_batch() -> List[Dict[str, Any]]:
        """
        Read all available commands from stdin (batch mode)
